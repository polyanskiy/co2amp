...
8  & $[1\,1^{1e}0,\,0\,3^{1e}0]_{II}$  & $u$ & 3/16 & \\
9  & $[1\,1^{1f}0,\,0\,3^{1f}0]_{I}$  & $u$ & 3/16 & \\
10 & $[1\,1^{1f}0,\,0\,3^{1f}0]_{II}$  & $u$ & 3/16 & \\
11 & $0\,3^{3e}0$  & $u$ & 1/8 & 11, 12: not currently included in the  \\
12 & $0\,3^{3f}0$  & $u$ & 1/8 & amplification model \\
[1.5ex]
...
16 & $0\,2^{2e}1$  & $u$ & 1/6 & \\
17 & $0\,2^{2f}1$  & $u$ & 1/6 & \\
\multicolumn{5}{c}{\boldmath{$0\,1\,2$} (Group 6)} \\
 &  &  &  & Reserved for future use \\
[1.5ex]
\multicolumn{5}{c}{\boldmath{$1\,1\,1 + 0\,3\,1$} (Group 7)} \\
 &  &  &  & Reserved for future use \\
[1.5ex]
\multicolumn{5}{c}{\boldmath{$0\,0\,3$} (Group 8)} \\
[1.5ex]
 &  &  &  & Reserved for future use \\
 [1.5ex]
\multicolumn{5}{c}{\boldmath{$1\,0\,2 + 0\,2\,2$} (Group 9)} \\
[1.5ex]
 &  &  &  & Reserved for future use \\
\hline
\end{tabular}
...
\begin{itemize}
    ...
    \item For asymmetric isotopologues, there are no symmetry restrictions due to the lack of identical nuclei, so all rotational levels with \( J \geq l \) are populated, and \( s(J) = 1 \).
\end{itemize}
...
N_{0\,1\,2}           &=   \frac{N}{\mathcal{Q}} \exp\left(-\frac{960}{T_2}\right) \exp\left(-\frac{2 \times 3380}{T_3}\right), \\
N_{1\,1\,1 + 0\,3\,1} &= 2 \frac{N}{\mathcal{Q}} \exp\left(-\frac{3 \times 960}{T_2}\right) \exp\left(-\frac{3380}{T_3}\right), \\
N_{0\,0\,3}           &=   \frac{N}{\mathcal{Q}} \exp\left(-\frac{3 \times 3380}{T_3}\right),\\
N_{1\,0\,2 + 0\,2\,2} &= 2 \frac{N}{\mathcal{Q}} \exp\left(-\frac{2 \times 960}{T_2}\right) \exp\left(-\frac{2 \times 3380}{T_3}\right).
\end{aligned}
\end{equation}
...
The intra-mode vibrational energy thermalization process is typically very fast. However, if the partial pressure of \ce{CO2} is very low, or if the active medium is pumped by a very short optical pulse, the finite thermalization time must be considered. The thermalization time, $\tau_V$, is assumed to be the same for all vibrational modes and is adopted from~\cite{Finzi-1975}, where the relaxation time of the $(10^01)$ and $(02^01)$ levels is given as:

\begin{equation}\label{eq:tauV}
\tau_V~[\mathrm{s}] = \frac{10^{-6}}{750\,(3.9\,P_{\ce{CO2}})},
\end{equation}

where the pressure $P_{\ce{CO2}}$ is measured in bar. Note that since intra-mode vibrational energy thermalization only involves collisional energy transfer between \ce{CO2} molecules, the thermalization time does not depend on the partial pressures of other gases, such as \ce{He} and \ce{N2}.
...
&p_{e4} = 0, \\
&p_{e3} = \begin{cases}
  \Phi \sigma   & \text{direct excitation of } (00^01) \text{ at } \sim 4.3~\mu\text{m}, \\
  \Phi \sigma   & \text{excitation via } (10^01, 02^01) \text{ at } \sim 2.8~\mu\text{m}, \\
  \Phi \sigma   & \text{excitation via } (20^01, 12^01, 04^01) \text{ at } \sim 2.0~\mu\text{m}, \\
  2 \Phi \sigma & \text{excitation via } (00^02) \text{ at } \sim 2.2~\mu\text{m}, \\
  3 \Phi \sigma & \text{excitation via } (00^03) \text{ at } \sim 1.4~\mu\text{m}.
\end{cases} \\
&p_{e2} = \begin{cases} 
  ...
  0             & \text{excitation via } (00^02) \text{ at } \sim 2.2~\mu\text{m}, \\
  0             & \text{excitation via } (00^03) \text{ at } \sim 1.4~\mu\text{m}.
\end{cases}
\end{aligned}
\end{equation}
...
\begin{aligned}
&\tau_2[\text{s}] = \frac{10^{- 6}}{\pi \times 7.61 \times 750 \times (P_{CO2}+0.73P_{N2}+0.64P_{He})},\\
