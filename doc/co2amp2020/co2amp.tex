...
%\usepackage[backend=biber,style=chem-angew,citestyle=numeric,sorting=none]{biblatex}
...
\subitem Staging (program output as an input for the next stage)
...
The simulation core \textbf{co2amp} and the user's interface shell \textbf{co2am+} are written in C++ programming language. \textbf{co2am+} uses the QT library (\url{http://qt.io}), and QT Creator (a part of the QT project) is used as a development environment. Windows executables are built using the MinGW compiler obtained as a part of the open-source QT distribution. The code is published in the GitHub website (\url{https://github.com/polyanskiy/co2amp}) and is freely available for use, modification, and redistribution under the terms of the GNU General Public License (GPL~v.3) (\url{https://www.gnu.org/licenses/gpl-3.0.html}). A binary package is provided in the form of a Windows installer that contains pre-compiled executables, documentation, templates and examples at \url{https://github.com/polyanskiy/co2amp/releases/}. The project only uses cross-platform libraries and thus should compile under other platforms (MacOS, Linux). \textbf{co2amp} depends on three third party components: gnuplot, 7-zip and HDF5 that are freely available for multiple platforms at \url{http://www.gnuplot.info/}, \url{https://www.7-zip.org/} and \url{https://www.hdfgroup.org/solutions/hdf5/} respectively. These components must be installed separately. The windows installer is built using the Nullsoft Scriptable Install System (NSIS, \url{https://nsis.sourceforge.io/}), and this is the only platform-specific part of the project. Finally, the documentation is mostly written in \LaTeX (http://www.latex-project.org) using the Overleaf online editor and compiler (\url{https://www.overleaf.com/}). YAML and HDF5 file formats are adopted for the specification of input parameters and storing the output field information respectively.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{co2amp and co2am+}
\textbf{co2amp} is a terminal program that allows simulating propagation of ultrashort pulses through an arbitrary cylindrically-symmetric optical system that can include {CO$_2$} amplifiers. It takes inputs in the form of specially formatted text files and command line arguments and produces outputs in the form of tabulated data files and a binary file with the complete information on the output field. \textbf{co2amp} can be used independently, however a graphical user interface makes the task of managing the program's inputs and outputs much more convenient.

\textbf{co2am+} is a graphical user interface program that simplifies the process of working with multiple input- and output- files and calculation parameters by keeping them organized and easily accessible via an intuitive working environment. \textbf{co2am+} allows saving/recalling the entire file structure of a project and command line parameters in a single compressed '.co2' file.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Projects}
The \textbf{co2amp} input parameters include the characteristics of the initial \texttt{pulse(s)}, the optical \texttt{layout} configuration, specifications for all \texttt{optics} used in the model (including laser amplifiers), and calculation parameters (e.g. calculation grid definition).
...
All \textbf{co2amp} inputs and outputs for a certain model constitute a project.

\textbf{co2am+} allows storing all inputs and outputs of the model except the output field in a single compressed project file with a '.co2' extension. Complete pulse information (complex field in every node of the space-time calculation grid) at the output of the system can be saved separately as a binary HDF5 file ('.pulse' extension) and used as an input for another project. An example of the input file structure of a '.co2' project accessed via the \textbf{co2am+} interface is shown in Fig.~\ref{fig:gui-input}.

\begin{figure}[ht]
 \centering
 \includegraphics[width=14cm]{images/gui-input}
 \caption{"Input" tab of the \textbf{co2am+} user interface program. YAML files specifying the \texttt{pulse}, \texttt{layout} and \texttt{optics} are listed on the left. Content of a selected file is displayed and can be edited in the big edit box on the right.}
 \label{fig:gui-input}
\end{figure}
...
A \texttt{pulse} is a complex electric field defined in every node of the calculation grid. A project can include one or more input pulses. Each one of them is defined in a separate YAML ('.yml') file. Either a reference to an output from another project (a '.pulse' file) or an explicit specification of the pulse's spatial- and temporal- profile can be used to define an input pulse.

The optical \texttt{layout} consists of a series of infinitely-thin \texttt{optics} separated by free space. \texttt{Pulses} propagate freely between the \texttt{optic}. A project must have one and only one \texttt{layout}. The \texttt{layout} is defined in a '.yml' file that specifies the order of the \texttt{planes} and distances between them.

An \texttt{optic} is an element of a system that alters the pulse when it passes through it. There are several types of \texttt{optic} described in some detail later. For example, a \textit{Lens} is an \texttt{optic} that introduces a radial-coordinate-dependent frequency shift changing the divergence of the beam. Each \texttt{optic} is specified in a separate '.yml' file. One \texttt{optic} can be used several times in the same \texttt{layout}. In this case \texttt{pulses} pass through the \texttt{optic} several times as, for example, in a laser cavity\footnote{Internally, \textbf{co2amp} code uses an additional concept: a \texttt{plane}. A \texttt{plane} is an element of a \texttt{layout} that, unlike an \texttt{optic}, only appears in the \texttt{layout} once. An \texttt{optic} is then associated with each \texttt{plane}. A \texttt{plane} is basically a placeholder for an \texttt{optic}.}.

\textbf{co2amp} supports seven types of \texttt{optics} listed in the Table~\ref{table:optics}
...
&&\\
M & \textit{Material} & A layer of a material. May introduce linear- and/or non-linear dispersion and/or absorption.\\
&&\\
C & \textit{Chirper} & An \texttt{optic} that applies a chirp to the pulse. Typically stretcher or compressor.\\
\hline
\end{tabularx}
\end{table}
...
The \texttt{pulse} is defined as a complex electric field in the nodes of a 2-dimensional space-time calculation grid moving with the pulse. Calculation grid is mostly defined via \textbf{co2amp} command line arguments. The only exception is the maximum radial coordinate that is equal to the semi-diameter of the clear aperture of an \texttt{optic} and thus changes from \texttt{optic} to \texttt{optic}. The command line arguments associated with the pulse space-time calculation grid are the numbers of nodes ("precision") in the pulse time- and radial coordinate- grids, minimum- and maximum- time limits and the central frequency. The central frequency is needed to unambiguously define the calculation grid in the frequency domains. 

Pulse time frame is used for all \texttt{pulse} related calculations (interaction with \texttt{optics}, free-space propagation) and for fast processes in some \texttt{optics}, e.g. fast molecular dynamics (stimulated transitions and rotational relaxation in an \textit{Active medium}). Processes that are much slower than the duration of the pulse (e.g. pumping of the active medium and vibrational relaxation) are modeled separately in a slower laboratory time-frame. Time-tick of the laboratory time-frame is also defined via a \textbf{co2amp} command line argument.

In \textbf{co2am+} the \textbf{co2amp} command line arguments are specified in the "Process" tab (Fig.~\ref{fig:gui-process}).
\begin{figure}[ht]
 \centering
 \includegraphics[width=14cm]{images/gui-process}
 \caption{"Process" tab of the \textbf{co2am+} user interface program. Values of \textbf{co2amp} command line arguments are specified on the left. \textbf{co2amp} output is displayed in the black text box on the right.}
 \label{fig:gui-process}
\end{figure}
The number of nodes in both coordinates of the pulse space-time frame always is a power of two that allows the use of Fast Fourier Transform (FFT) algorithms. Calculations with a larger number of nodes  usually are more accurate but, on the other hand, take longer and require more computer memory (both calculation time and required memory are roughly proportional to the product of the number of  nodes in the time and space grids). Therefore it  is recommended to start  running the simulation with a smaller number of nodes and then repeat it several times, each time with a denser grid. The absence of considerable change in the program’s output with an increase in the number of nodes will indicate that the density of the grid is satisfactory.

The time-step, $\Delta t=(t_{max}-t_{min})/N_t$, where $t_{max}$ and $t_{min}$ define the time range and $N_t$ is the number of nodes in the time grid, must be small enough to accurately describe the pulse profile at all stages of its  propagation through the optical system. It also is important to remember that the time range and number of nodes in the time grid also define the range and step in the frequency domain: $\Delta\nu=1/(t_{max}-t_{min})$ and $(\nu_{max}-\nu_{min})=1/\Delta t$. This means that the time range must be long enough to provide sufficient resolution in the frequency domain, while, concurrently, the time step must be sufficiently short to provide a bandwidth that fits the entire spectral region of interest.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Units}
SI units without prefixes, e.g. "meters, seconds, Amperes..." but not "centimeters, nanoseconds, kiloamperes...", are used in \textbf{co2amp} input and output and also in the \textbf{co2amp} code internally. \textbf{co2amp+} allows changing the units used for the graphical representation of the calculation results on the "Output" tab (Fig.~\ref{fig:gui-output}).
\begin{figure}[ht]
 \centering
 \includegraphics[width=14cm]{images/gui-output}
 \caption{"Output" tab of the \textbf{co2am+} user interface program. Controls on the left allow selecting the data to display and fine-tuning the look of the plots.}
 \label{fig:gui-output}
\end{figure}
However, when numerical data are accessed via [Right-click on a plot] -- [Copy raw data] the units of the data are always "prefix-less".


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Program output}
The output of the program is given as a temporal and spatial structure of every \texttt{pulse} at each \texttt{optic} in the \texttt{layout}. Temporal (and spectral) profiles are integrated on the entire area of the \texttt{optic}, and spatial profiles are integrated on the duration of the pulse time-frame.

In the \textbf{co2am+} "Output" tab the user can chose a \texttt{pulse} and an \texttt{optic} to display (Fig.~\ref{fig:gui-output}). If the selected \texttt{optic} is used several times in the \texttt{layout}, it also is possible to specify which passes through the \texttt{optic} will be displayed. Also, the integral pulse energy can be provided either at every pass through a selected \texttt{optic}, or at all passes through all \texttt{optics} of the \texttt{layout}.

Output for some types of \texttt{optics} supplies additional type-specific information. For instance, for \textit{Active medium}, this includes gain, discharge profile, population dynamics, and the dynamics of the distribution of pumping energy (fractions of discharge energy going into the excitation of laser levels, excitation of molecular translations and ionization). Output for the optic of type \textit{Probe} include the information of the phase of the optical field in the center of the beam.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{"Comments" and "About" tabs of co2am+}
The "Comments" tab provides an edit box to enter any comments about the project that will be stored as a part of the project in the '.co2' file.

The "About" tab contains information about the versions of the \textbf{co2amp} and \textbf{co2am+} programs, links to the license and the documentation (this file), author contact information, and a suggested citation format.
...
A project must include the following elements specified in the input YAML files:
\begin{enumerate}
    \item One or more \texttt{pulses}
    \item One or more \texttt{optics}
    \item One \texttt{layout}
\end{enumerate}
Each element is specified in a dedicated YAML ('.yml') file\footnote{\textbf{co2amp} also requires an additional input file 'config\_files.yml' that lists all input YAML files and the types of corresponding elements. \textbf{co2am+} creates this file automatically.}. The last \texttt{optic} in the \texttt{layout} must be of type P (\textit{Probe}).

In the following sections we briefly describe each of these elements and discuss models associated with them. This description is not complete. Refer to the templates and the example files (and the comments in them) for more information.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{\texttt{Pulse}}
Unless the output of another project ('.pulse' file) is used as an input, both the temporal- and spatial- shape of the input \texttt{pulse} must be specified in a corresponding YAML ('.yml') file. The \texttt{pulse} is assumed to be transform-limited (no initial chirping). The \texttt{pulse} energy, central frequency, and the injection time must also be specified. The injection time specifies the time-delay between the zero moment of the lab time frame ("slow" time frame) and injecting a \texttt{pulse} into the optical system (first \texttt{optic} in the \texttt{layout}). An example of a \texttt{pulse} configuration file is shown below.
...
This file specifies a 2~ps (FWHM) transform-limited Gaussian pulse with an $w=3$~mm Gaussian beam profile, 1~mJ energy and 32.5~THz central frequency injected into the system at $t_{in}=0$. There are several pre-defined beam- and pule- profile options: \texttt{GAUSS}, \texttt{FLATTOP}, \texttt{SUPERGAUSS4}, \texttt{SUPERGAUSS6} etc. Alternatively, a \texttt{FREEFORM} option can be selected followed by a tabulated numerical specification of an arbitrary shape (see 'pulse.yml' template for details). 


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{\texttt{Layout}}
\subsection{Configuration}
The \texttt{layout} configuration defines the order of \texttt{optics} and the distances between them. A simple \texttt{layout} configuration file looks like this:
...
Here, the system consists of two \texttt{optics}: \texttt{P1} and \texttt{P2} separated by 3 meters of free space. The pulse(s) pass through the system once. If the \texttt{times} value is more than 1, a pulse after passing through \texttt{P2} will go to \texttt{P1} again, and the propagation through the system will repeat the specified number of times. There can be several "go-times" in a \texttt{layout} configuration file. An example of a \texttt{layout} configuration for a more complex system is shown below
...
In the \textbf{co2amp} model, the \texttt{optics} are infinitely thin. In a case of a long \texttt{optic}, for example an \textit{Active medium}, the model first calculates the field modification accumulated by a \texttt{pulse} when it propagates through the \texttt{optic} and then apply it as if it occurred at once. This approach may not work well if the actual optical element is long and the \texttt{pulse} changes considerably while propagating through it, thus interacting differently with different parts of the \texttt{optic}. Accuracy of the model can be improved if long elements are divided into shorter sub-sections.

Fig.~\ref{fig:layout} shows an example of a 2-meter long \texttt{layout} with a meter long active medium in the middle. In one case shown in Fig.~\ref{fig:layout}a we first propagate the\texttt{pulse} to the middle of the amplifier section, then apply the amplification accumulated over 1~meter and propagate the\texttt{pulse} to the last \texttt{optic}.
\begin{figure}[ht]
\centering
\includegraphics{images/layout}
\caption{Example of \texttt{layout} configuration for a long \texttt{optic} (in this case an \textit{Active medium}. a) The \textit{Active medium} is represented by a single \texttt{optic}. b) The \textit{Active medium} is split in two shorter sections.}\label{fig:layout}
\end{figure}
...
Alternatively, we can represent the active medium by two 0.5-meter sections as shown in Fig.~\ref{fig:layout}b. The corresponding layout is

\begin{verbatim}
#=========================
# long amplifier is divided in two shorter sections
...
%%%%%%%%%%%%%
\subsection{Modelling of \texttt{pulse} propagation between \texttt{optics}}\label{propagation}

Consider free-space wave propagation between plane-parallel surfaces $S'$ and $S$ separated by distance $z$ as shown in Fig.~\ref{fig:diffraction} for the case of cylindrical symmetry.
\begin{figure}[ht] 
 \centering
 \includegraphics[width=10cm]{images/diffraction.pdf}
 \caption{Application of Huygens-Fresnel principle to beam propagation from plane $S'$ to plane $S$ for a system with cylindrical symmetry.}
 \label{fig:diffraction}
\end{figure}
According to Huygens-Fresnel principle, the field $E$ in a point on the plane $S$  is defined as a superposition of secondary waves emitted from every point of plane $S'$~\cite{BornWolf:1999}. This can be written in the case of cylindrical symmetry as~\cite{Siegman:1986,PeatrossWare:2015}
\begin{subequations} \label{eq:DI}
 \begin{align}
  &E(\rho) = -\frac{i}{\lambda} \int_{\rho'=0}^{\infty} E'(\rho') \int_{\phi'=0}^{2\pi} \frac{e^{ikR}}{R} K d\phi' \rho' d\rho'
  \label{eq:DI2a}\\
  &R = \sqrt{\rho^2 + \rho'^2 + z^2 -2\rho\rho'\cos\phi'}
  \label{eq:DI2b}\\
  &K = \cos\theta = \frac{z}{R}
  \label{eq:DI2c}
 \end{align}
...
Note that because the field on the output plane $S$ doesn't depend on the angular coordinate $\phi$, $\phi=0$ is chosen for the simplification of Eq.~(\ref{eq:DI}).

Direct numerical integration of Eq.~(\ref{eq:DI}) having $O(N^3)$ complexity is very time consuming and thus an approximation is usually used to accelerate computations. Fresnel diffraction is the most known approximation to the diffraction integral. The following assumptions are done in the Fresnel approximation
\begin{equation} \label{eq:FreA}
 ...
 \end{split}
\end{equation}
where "denominator" and "exponent" signify the position of the $R$ variable in Eq.~(\ref{eq:DI2a}).

After substituting Eq.~(\ref{eq:FreA}) to Eq.~(\ref{eq:DI2a}) and using the following formula
\begin{equation} \label{eq:formula}
 \int_{0}^{2\pi} e^{\pm i a \cos\phi}  d\phi = 2 \pi J_0(a)
\end{equation}
where $J$ is the Bessel function we get the expression for Fresnel diffraction with cylindrical symmetry
...
\textbf{co2amp} supports both Rayleigh-Sommerfeld- Eq.~(\ref{eq:DI}) and Fresnel- Eq.~(\ref{eq:Fre}) based propagation methods. Also, the user can chose to ignore the \texttt{pulse} evolution during free-space propagation.

Eq.~(\ref{eq:DI}) and Eq.~(\ref{eq:Fre}) assume a monochromatic light, which is not the case for ultrashort pulses that have non-negligible bandwidth. Therefore, in \textbf{co2amp} the propagation is calculated in frequency domain: Eq.~(\ref{eq:DI}) or Eq.~(\ref{eq:Fre}) are applied to Fourier-transformed field in every node of the frequency calculation grid. After that an inverse Fourier transform is used to return to the time domain.



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{\texttt{Optic} type A \textit{Active medium}}
\textit{Active medium} is by far the most complex \texttt{optics} that can be used in a \textbf{co2amp} project. The models used for simulating the molecular dynamics and \texttt{pulse} amplification are described in a dedicated Chapter~\ref{chapter:models}.

Configuration file for an \texttt{optic} of type A must include specifications of the gas mixture, pumping, and laser transitions included in the simulations. An example of a configuration file is shown below

\begin{verbatim}
#=========================
# AM1.yml from 'examples/ATF 5 TW/ATF 3 final ampifier.co2' project
...
The composition (including isotopic enrichment of carbon dioxide) and the initial temperature of the active medium are specified under the "\#Gas mixture (bar)" comment.

For discharge pumping, the geometry of the discharge and its profile must be given. For optical pumping, the wavelength, the absorption cross-section, and the temporal profile of the pumping pulse must be provided.

Comments in the 'optic~A~(discharge~pumped~CO2~amplifier).yml' and\\ 'optic~A~(optically~pumped~CO2~amplifier).yml' template files provide a detailed information on the format of the configuration file.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{\texttt{Optic} type P \textit{Probe}}
\textit{Probe} is a passive \texttt{optic}. It doesn't change the field that fits inside its semi-diameter.

\begin{equation}
E(t,\rho) = E'(t,\rho)
\end{equation}
where $E'(t,\rho)$ and $E(t,\rho)$ is the field before- and after- an \texttt{optic} respectively.

However, a \textit{Probe} \texttt{optic} can be used as a limiting aperture that has a zero transmittance for $\rho>semiDia$. The only configuration parameter for an \texttt{optic} of type P is its semi-diameter. A configuration file for a 25~mm semi-diameter \textit{Probe} is shown below
...
\textit{Spatial filter} applies a specified coordinate-dependent transmittance function to a \texttt{pulse}

\begin{equation}
E(t,\rho) = E'(t,\rho) \sqrt{\mathcal{T}(\rho)}
\end{equation}
where $\mathcal{T}(\rho)$ is the transmittance function defined in the configuration file.
...
See the 'optic~F~(spatial~filter).yml' template file for details

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{\texttt{Optic} type S \textit{Spectral filter}}
\textit{Spectral filter} applies a specified frequency-dependent transmittance function to a \texttt{pulse}
...
See the 'optic~S~(spectral~filter).yml' template file for details



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{\texttt{Optic} type L \textit{Lens}}
\textit{Lens} is a lens.
...
The calculation is done in frequency domain to ensure that effective focal length is same for all frequencies in the pulse spectrum.

Configuration example (1-meter focal length lens):
...
In the case of an oblique incidence, effective intensity $I_{eff}$ is reduced and propagation distance in the material (effective thickness) $\Theta_{eff}$ is increased automatically depending on the incidence angle $\theta_i$ and the refractive index $n$ of the material

\begin{equation}
 \begin{aligned}
   &\theta_r = \arcsin\left(\frac{\sin\theta_i}{n_0}\right)\\
   &I_{eff} = I \frac{\cos\theta_i}{\cos\theta_r}\\
   &\Theta_{eff} = \frac{\Theta}{\cos\theta_r} 
 \end{aligned}
\end{equation}
where $I$ and $\Theta$ are the intensity before the \texttt{optic} and the actual thickness of the material and $\theta_r$ is the refraction angle.
...
\begin{equation}
 \frac{dt}{d\nu'} = \frac{\Theta_{eff}}{c} \frac{dn_{g}}{d\nu'},
\end{equation}
$\nu_c$ is the central frequency.

where $\widehat{E}(\nu,\rho)$ is the field in the frequency domain, $\mathcal{F}$ and $\mathcal{F}^{-1}$ are the Fourier-transform and inverse-Fourier-transform functions, $\nu$ is the frequency, $\nu_c$ is the central frequency, $n_{g}$ is the group index of refraction. The dispersion formulas used for calculating $n_{g}$ are given in Appendix~\ref{appendix:optical_constants}.

\subsubsection{Nonlinear interaction}
\begin{equation}
 \begin{aligned}
  &E(t,\rho) = E'(t,\rho) \exp\left(2\pi i \nu_c \frac{\Theta_{eff}}{c} n_2 I_{eff}(t,\rho)\right)\\
  &I_{eff}(t,\rho) = 2 h \nu_c (E'(t,\rho))^2 \frac{\cos\theta_i} {\cos\theta_r}
 \end{aligned}
\end{equation}
where $n_2$ is the nonlinear refractive index, $h$ the Plank's constant, and $I(t,r)$ is the field intensity. The numerical values of $n_2$ used in the program are given in Appendix~\ref{appendix:optical_constants}.
...
The following materials are currently supported: CdTe, GaAs, Ge, KCl, NaCl, Si, ZnSe, and air. An arbitrary $n_2$ can be specified in the configuration file. A pre-defied value (Appendix~\ref{appendix:optical_constants}) is used otherwise. To improve accuracy, the \textit{Material} \texttt{optic} can be split in several layers. A split-step method is used to calculate linear- and non-linear interaction with a layer: first, a non-linear interaction with a half-layer is calculated, then a full-layer linear interaction, and finally a half-layer nonlinear interaction again. 


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{\texttt{Optic} type C \textit{Chirper}}
\textit{Chirper} introduces a chirp to a pulse and is typically used to model a stretcher or compressor.
...
In the case of linear chirp the chirpyness is constant and and Eq.~(\ref{eq:Delta_nu}) becomes
...
Only linear chirp is presently supported.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Modelling of processes in \texorpdfstring{CO\textsubscript{2}}{CO2} amplifiers}\label{chapter:models}
...
\end{aligned}
\end{equation}
where $W(t)$ is the discharge power density measured in {kW/cm$^3$} , $p_e$ is measured in {{\textmu}s$^{-1}$}, and the constants $k$ are calculated using the following expressions \cite{Biryukov-1974,Taylor-1969}:
...
In the approximation used in the \texttt{co2amp} model, the processes of pumping and vibrational relaxation are slow compared to the duration of the pulse. Thus, only the stimulated transitions contribute to the change of the populations of vibrational levels during the pulse.
...
The following expressions and values for linear ($n_0$) and nonlinear ($n_2$) refractive indexes and linear absorption ($\alpha_0$) are used in the program (wavelength $\lambda$ in the formulas must be expressed in {\textmu}m):

\subsection*{AgBr}
$n_0=\sqrt{3.860 + \frac{0.8677\lambda^2}{\lambda^2-0.3211^2} + \frac{21.61\lambda^2}{\lambda^2-254.2^2}}$ ($\lambda$ = 0.495--12.67~{\textmu}m) - Fit of data from \cite{Schroter-1931} and \cite{McCarthy-1973}\\
\\
$n_2 = 2.0\times 10^{-19}$ m$^2$/W at 9.2 {\textmu}m - Preliminary data

\subsection*{AgCl}
$n_0=\sqrt{4.00804+\frac{0.079086}{\lambda^2-0.04584}-0.00085111\lambda^2-0.00000019762\lambda^4}$ ($\lambda$ = 0.578--20.6~{\textmu}m) \cite{Tilton-1950}\\
\\
$n_2 = 1.7\times 10^{-19}$ m$^2$/W at 9.2 {\textmu}m - Preliminary data

\subsection*{AMTIR1 (IRG 22)}
$n_0=\sqrt{3.4834+\frac{2.8203\lambda^2}{\lambda^2-0.1352}+\frac{0.9773\lambda^2}{\lambda^2-1420.7}}$ ($\lambda$ = 0.8--15.5~{\textmu}m) \cite{AMTIR1}

\subsection*{BaF$_2$}
$n_0=\sqrt{1.33973+\frac{0.81070\lambda^2}{\lambda^2-0.10065^2}+\frac{0.19652\lambda^2}{\lambda^2-29.87^2}+\frac{4.52469\lambda^2}{\lambda^2-53.82^2}}$ ($\lambda$ = 0.15--15~{\textmu}m) \cite{Li-1980}\\
\\
$n_2 = 1.7\times 10^{-20}$ m$^2$/W at 9.2 {\textmu}m \cite{Polyanskiy-2021b}\\
\\
$\alpha_0 = 0.8 (e^{1.20(\lambda-8)}-1)$ m$^{-1}$ - Preliminary data

\subsection*{CdTe}
$n_0=\sqrt{1+\frac{6.1977889\lambda^2}{\lambda^2-0.1005326}+\frac{3.2243821\lambda^2}{\lambda^2-5279.518}}$ ($\lambda$ = 6--22~{\textmu}m) \cite{DeBell-1979}\\
\\
$n_2 = -2.95\times 10^{-17}$ m$^2$/W at 1.06 {\textmu}m \cite{Sheik-Bahae-1991}

\subsection*{CsI}
$n_0=\sqrt{1.27587+\frac{0.68689\lambda^2}{\lambda^2-0.130^2}+\frac{0.26090\lambda^2}{\lambda^2-0.147^2}+\frac{0.06256\lambda^2}{\lambda^2-0.163^2}+\frac{0.06527\lambda^2}{\lambda^2-0.177^2}+\frac{0.14991\lambda^2}{\lambda^2-0.185^2}+\frac{0.51818\lambda^2}{\lambda^2-0.206^2}+\frac{0.01918\lambda^2}{\lambda^2-0.218^2}+\frac{3.38229\lambda^2}{\lambda^2-161.29^2}}$ ($\lambda$ = 0.25--67~{\textmu}m) \cite{Li-1976}\\
\\
$n_2 = 1.2\times 10^{-19}$ m$^2$/W - Preliminary data

\subsection*{GaAs}
$n_0=\sqrt{5.372514+\frac{5.466742\lambda^2}{\lambda^2-0.4431307^2}+\frac{0.02429960\lambda^2}{\lambda^2-0.8746453^2}+\frac{1.957522\lambda^2}{\lambda^2-36.9166^2}}$ ($\lambda$ = 0.97--17~{\textmu}m) \cite{Skauli-2003}\\
\\
$n_2 = -3.26\times 10^{-17}$ m$^2$/W at 1.06 {\textmu}m \cite{Sheik-Bahae-1991}

\subsection*{Ge}
$n_0=\sqrt{1+\frac{0.4886331\lambda^2}{\lambda^2-1.393959}+\frac{14.5142535\lambda^2}{\lambda^2-0.1626427}+\frac{0.0091224\lambda^2}{\lambda^2-752.190}}$ ($\lambda$ = 2--14~{\textmu}m) \cite{Burnett-2016}\\
\\
$n_2 = 2.83\times 10^{-17}$ m$^2$/W at 10.6 {\textmu}m \cite{Sheik-Bahae-1991}

\subsection*{KBr}
$n_0=\sqrt{1.39408+\frac{0.79221\lambda^2}{\lambda^2-0.146^2}+\frac{0.01981\lambda^2}{\lambda^2-0.173^2}+\frac{0.15587\lambda^2}{\lambda^2-0.187^2}+\frac{0.17673\lambda^2}{\lambda^2-60.61^2}+\frac{2.06217\lambda^2}{\lambda^2-87.72^2}}$ ($\lambda$ = 0.2--42~{\textmu}m) \cite{Li-1976}\\
\\
$n_2 = 4.3 \times 10^{-20}$ m$^2$/W at 9.2 {\textmu}m - Preliminary data

\subsection*{KCl}
$n_0=\sqrt{1.26486+\frac{0.30523\lambda^2}{\lambda^2-0.100^2}+\frac{0.41620\lambda^2}{\lambda^2-0.131^2}+\frac{0.18870\lambda^2}{\lambda^2-0.162^2}+\frac{2.6200\lambda^2}{\lambda^2-70.42^2}}$ ($\lambda$ = 0.18--35~{\textmu}m) \cite{Li-1976}\\
\\
$n_2 = 3.4 \times 10^{-20}$ m$^2$/W at 9.2 {\textmu}m \cite{Polyanskiy-2021b}

\subsection*{NaCl}
$n_0=\sqrt{1.00055+\frac{0.19800\lambda^2}{\lambda^2-0.050^2}+\frac{0.48398\lambda^2}{\lambda^2-0.100^2}+\frac{0.38696\lambda^2}{\lambda^2-0.128^2}+\frac{0.25998\lambda^2}{\lambda^2-0.158^2}+\frac{0.08796\lambda^2}{\lambda^2-40.50^2}+\frac{3.17064\lambda^2}{\lambda^2-60.98^2}+\frac{0.30038\lambda^2}{\lambda^2-120.34^2}}$\\($\lambda$ = 0.2--30~{\textmu}m) \cite{Li-1976}\\
\\
$n_2 = 3.5 \times 10^{-20}$ m$^2$/W at 9.2 {\textmu}m \cite{Polyanskiy-2021b}

\subsection*{NaF}
$n_0=\sqrt{1.0.41572+\frac{0.32785\lambda^2}{\lambda^2-0.117^2}+\frac{3.18248\lambda^2}{\lambda^2-40.57^2}}$ ($\lambda$ = 0.15--17~{\textmu}m) \cite{Li-1976}\\
\\
$n_2 = 0.6 \times 10^{-20}$ m$^2$/W at 9.2 {\textmu}m - Preliminary data\\
\\
$\alpha_0 = 5.0 (e^{0.97(\lambda-8)}-1)$ m$^{-1}$ - Preliminary data

\subsection*{Si}
$n_0=3.41983+\frac{0.159906}{\lambda^2-0.028}-0.123109\left(\frac{1}{\lambda^2-0.028}\right)^2+1.26878\times 10^{-6}\lambda^2-1.95104\times 10^{-9}\lambda^4$\\($\lambda$ = 2.44--25~{\textmu}m) \cite{Edwards-1980}\\
\\
$n_2 = 1.0\times 10^{-17}$ m$^2$/W at 2.2 {\textmu}m \cite{Bristow-2007}

\subsection*{SiO$_2$}
$n_0=\sqrt{1+\frac{0.6961663\lambda^2}{\lambda^2-0.0684043^2}+\frac{0.4079426\lambda^2}{\lambda^2-0.1162414^2}+\frac{0.8974794\lambda^2}{\lambda^2-9.896161^2}}$ ($\lambda$ = 0.21--6.7~{\textmu}m) \cite{Malitson-1965}\\
\\
$n_2 = 3.29\times 10^{-20}$ m$^2$/W at 1.06 {\textmu}m \cite{Sheik-Bahae-1991}

\subsection*{ZnS}
$n_0=\sqrt{8.393+\frac{0.14383}{\lambda^2-0.2421^{2}}+\frac{4430.99}{\lambda^2-36.71^{2}}}$ ($\lambda$ = 0.405--13~{\textmu}m) \cite{Klein-1986}\\
\\
$n_2 = 2.5\times 10^{-19}$ m$^2$/W at 9.2 {\textmu}m - Preliminary data

\subsection*{ZnSe}
$n_0=\sqrt{1+\frac{4.45813734\lambda^2}{\lambda^2-0.200859853^2}+\frac{0.467216334\lambda^2}{\lambda^2-0.391371166^2}+\frac{2.89566290\lambda^2}{\lambda^2-47.1362108^2}}$ ($\lambda$ = 0.54--18.2~{\textmu}m) \cite{Tatian-1984}\\
$n_2 = 2.87\times 10^{-18}$ m$^2$/W at 1.06 {\textmu}m \cite{Sheik-Bahae-1991}

\subsection*{Air}
Refractive index $n_0$ is calculated using Mathar's model for $\lambda$ = 7.5--14~{\textmu}m \cite{Mathar-2007}\\
\\
$n_2 = 3\times 10^{-23}$ m$^2$/W at 9.2 {\textmu}m \cite{Polyanskiy-2021a}
...
Combining the constants and switching to kW/cm$^3$ for power density and {{\textmu}s$^{-1}$} for the rate constants we get the formulas given in the model description:
