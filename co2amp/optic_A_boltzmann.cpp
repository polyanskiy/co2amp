...
void A::Boltzmann(int m, double* q)
{
    double f[b0] = {};
    double Du;
    double u_lim;
    ...
    Debug(3, "Writing and solving Boltzmann equations - Stage 1 (coarse net)...");
    WriteAndSolveEquations(voltage[m], Du, f, q, false);
    ...
    Debug(3, "Writing and solving Boltzmann equations - Stage 1 (coarse net)...");
    WriteAndSolveEquations(voltage[m], Du, f, q, true);
    Debug(3, "Stage 2 completed");


    /*Debug(3, "Saving f (for debugging only...)");
    Save_f(Du, f); //test Boltzmann solver
    Debug(3, "Saving f completed");*/
}


void A::WriteAndSolveEquations(double U, double Du, double* f, double *q, bool calculate_q)
{
    ...
    InterpolateArray(Q_x, Q_y, std::size(Q_x), Du, Q);
    ...
    InterpolateArray(Qm1_x, Qm1_y, std::size(Qm1_x), Du, Qm1);
    ...
    InterpolateArray(Qm2_x, Qm2_y, std::size(Qm2_x), Du, Qm2);
    ...
    InterpolateArray(Qm3_x, Qm3_y, std::size(Qm3_x), Du, Qm3);
    ...
    InterpolateArray(Q11_x, Q11_y, std::size(Q11_x), Du, Q1[1]);
    ...
    InterpolateArray(Q21_x, Q21_y, std::size(Q21_x), Du, Q2[1]);
    ...
    InterpolateArray(Q22_x, Q22_y, std::size(Q22_x), Du, Q2[2]);
    ...
    InterpolateArray(Q23_x, Q23_y, std::size(Q23_x), Du, Q2[3]);
    ...
    InterpolateArray(Q24_x, Q24_y, std::size(Q24_x), Du, Q2[4]);
    ...
    InterpolateArray(Q25_x, Q25_y, std::size(Q25_x), Du, Q2[5]);
    ...
    double E = U/(D*1e2); // Electric field stretch, V/cm
    double N = 2.7e19*(p_CO2+p_N2+p_He); // Total molecule number density, 1/cm^3
    double E_over_N = E/N*1e16;
    ...
    for(int i=0; i<b0; ++i)
    {
        ...
        for(int k=1; k<11; k++)
        {
            int j = (int)floor((u[i]+u1[k])/Du + 0.5);
            if(j < b0)
                M[i][j] += Y1*u[j]*Q1[k][j];
        }
        for(int k=1; k<16; k++)
        {
            int j = (int)floor((u[i]+u2[k])/Du + 0.5);
            if(j < b0)
                M[i][j] += Y2*u[j]*Q2[k][j];
        }
    ...
    for(int i=1; i<b0; ++i)
    {
        ...
        if(M[i-1][i-1]==0.0)
        {
            std::cout << "Error !!!!!!!!!!!!!!!!!!! " << i-1 << std::endl << std::flush;
        }
...
void A::Save_f(double Du, double* f)
{
    FILE *file;
