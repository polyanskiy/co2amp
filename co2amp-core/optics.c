...
void Stretcher(int pulse, double stretching) // stretching: s/Hz
{
    ...
    for(x=0; x<x0; x++){
        ...
        for(n=0; n<n0; n++){
            ...
            delay *= 0.5; // conversion to phase delay. Don't know why "0.5" but it works
...
double RefractiveIndex(char* material, double nu)
{
    ...
    if(!strcmp(material, "diamond")){
        x= x<0.225 ? 0.225 : x;
        x= x>100 ? 100 : x;
        return sqrt( 1.0 + 4.3356*pow(x,2)/(pow(x,2)-pow(0.1060,2)) + 0.3306*pow(x,2)/(pow(x,2)-pow(0.1750,2)) );
    }
    if(!strcmp(material, "cdte")){
        x= x<6 ? 6 : x;
        x= x>22 ? 22 :x;
...
double NonlinearIndex(char* material)
{
    ...
    if(!strcmp(material, "kcl"))
        return 2e-13 * 4.19e-7 / RefractiveIndex(material, vc); // esu -> m^2/W
    if(!strcmp(material, "nacl"))
        return 1.6e-13 * 4.19e-7 / RefractiveIndex(material, vc); // esu -> m^2/W
    if(!strcmp(material, "znse"))
        return 170e-13 * 4.19e-7 / RefractiveIndex(material, vc); // esu -> m^2/W
    if(!strcmp(material, "ge"))
        return 2700e-13 * 4.19e-7 / RefractiveIndex(material, vc); // esu -> m^2/W
    if(!strcmp(material, "gaas"))
        //return -2700e-13 * 4.19e-7 / RefractiveIndex(material, vc); // esu -> m^2/W
    ...
    if(!strcmp(material, "diamond"))
        return 0; // need data!!!
    if(!strcmp(material, "cdte"))
        return -2000e-13 * 4.19e-7 / RefractiveIndex(material, vc); // esu -> m^2/W
